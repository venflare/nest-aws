...
export class NotificationsService {
  ...
  processThenDeleteQueueMessages(): void {
    this.loggerService.log('Fetching queue messages...');
    ...
    sqs.receiveMessage(
      paramsQueueReceiveMessage,
      (error: aws.AWSError, data: aws.SQS.ReceiveMessageResult): void => {
        if (error) {
          this.loggerService.error(`Receive queue message error => ${error}`);
        } else if (data.Messages) {
          const enqueueEmailDto: EnqueueEmailDto = JSON.parse(
            data.Messages[0].Body,
          );
          const messageReceiptHandle: string = data.Messages[0].ReceiptHandle;

          this.loggerService.log(
            `Receive queue message success => ${enqueueEmailDto}`,
          );
          ...
          const paramsEmailSendEmail: aws.SES.SendEmailRequest = {
            ...
            Message: {
              Body: {
                Text: {
                  Data: `Hi ${enqueueEmailDto.userGreetingName}, ${enqueueEmailDto.emailMessage}.`,
                },
              },
          ...
          ses.sendEmail(
            paramsEmailSendEmail,
            (error: aws.AWSError, data: aws.SES.SendEmailResponse): void => {
              if (error) {
                ...
                sqs.deleteMessage(
                  paramsQueueDeleteMessage,
                  (error: aws.AWSError, data: any): void => {
                    if (error) {
                      this.loggerService.error(
                        `Delete queue message error => ${error}`,
                      );
                    } else {
                      this.loggerService.log(
                        `Delete queue message success => ${data}`,
                      );
                    }
                  },
                );
